...
- **EMUZ80_RP2040_PCB 基板・ベースファームウェア** — tendai22plus氏 (https://github.com/tendai22)
...
| CP/M 2.2 CCP+BDOS (`ccp_bdos.*`) | Digital Research (1979) | Caldera により非商用再配布許可 (1998) |
| CP/M システムディスク (`cpm22-1.*`) | z80pack 同梱 | MIT (z80pack) / Caldera許可 (CP/M) |
| MS-BASIC等 (`cpm22_disk1.*`) | (C) Microsoft Corporation (1977,1985-1986) / Z80-MBC2同梱 | 非公式アーカイブ。歴史的保存および教育目的でのみ同梱。 |
| Turbo Pascal 3.01a (`cpm22_tp301a.*`) | Borland International (1983-1985) | Antique Software として無料公開 (Embarcadero) |
| Z80 fig-FORTH (`cpm22_z80forth.*`) | FORTH Interest Group (fig) | パブリックドメイン |
| HI-TECH C v3.09 (`cpm22_htc.*`) | HI-TECH Software / Microchip | フリー配布 (Microchip 公開) |

> [!IMPORTANT]
> **CP/M 2.2** バイナリは 1998年の Caldera, Inc. の許可に基づき再配布されています。
...
> **Turbo Pascal 3.01a** は Embarcadero (旧Borland) により "Antique Software" として無料公開されたものです。
> **HI-TECH C** コンパイラは Microchip Technology によりフリーで公開されたものです。
